 ...
 * 初始化错误回调
 */
void InitCallback(JNIEnv *env, jclass cls, int error, int type) {
    jmethodID jmethodid;
    if (type == 0) {
        jmethodid = (*env)->GetStaticMethodID(env, cls, "onInit", "(I)V");
    } else {
        jmethodid = (*env)->GetStaticMethodID(env, cls, "onExit", "(I)V");
    }
    if (jmethodid == NULL) {
        LOGE("event   jmethodid == null");
        return;
    }
    (*env)->CallStaticVoidMethod(env, cls, jmethodid, error);
}

int scan_dir(JNIEnv *env, jclass cls, const char *dir, int depth)   // 定义目录扫描函数
{
    DIR *dp;                      // 定义子目录流指针
    ...
    while ((entry = readdir(dp)) != NULL)  // 获取下一级目录信息，如果未否则循环
    {
        ...
        if (S_IFDIR & statbuf.st_mode)    // 判断下一级成员是否是目录
        {
            ...
            if (NULL != path1) {
                ...
                lwd = inotify_add_watch(lnotifyFD, path,
                                        IN_DELETE | IN_CREATE);//监听xxx目录下的 delete、create事件
                if (-1 == lwd) {
                    LOGE("-1 == LWD\n");
                    continue;
                }
            ...
            scan_dir(env, cls, entry->d_name, depth + 4);              // 递归调用自身，扫描下一级目录的内容
        }
    }
    chdir("..");                                                  // 回到上级目录
    closedir(dp);                                                 // 关闭子目录流
...
int *fileObserver_init(ST_MASK *stMask) {
    ...
    if (NULL == p) {
        ...
        a = -1;
        InitCallback(env, gl_class, errno, 0);
        return &a;
    }
    memset(p, 0, strlen(path) + 1);
    ...
    int ret = scan_dir(env, gl_class, path, 0);
    if (ret == 0)
        InitCallback(env, gl_class, 0, 0);
    while (RUN) {
        // 4.epoll检测文件的可读变化
        ...
        for (i = 0; i < pollResult; i++) {
            ...
            if (readCount < sizeof(inotifyEvent)) {
                ...
                InitCallback(env, gl_class, errno, 1);
                return &a;
            }
                ...
                if (curInotifyEvent->len > 0) {
                    if (curInotifyEvent->mask & IN_CREATE) {
                        if (pathName[mPendingEventItems[i].data.fd] != NULL) {
                            ProcessEvent(env, gl_class, pathName[mPendingEventItems[i].data.fd],
                                         curInotifyEvent->name, IN_CREATE);
                        } else {
                        ...
                        if (pathName[mPendingEventItems[i].data.fd] != NULL) {
                            ProcessEvent(env, gl_class, pathName[mPendingEventItems[i].data.fd],
                                         curInotifyEvent->name, IN_DELETE);
                        } else {
                        ...
                        if (pathName[mPendingEventItems[i].data.fd] != NULL) {
                            ProcessEvent(env, gl_class, pathName[mPendingEventItems[i].data.fd],
                                         curInotifyEvent->name, IN_ACCESS);
                        } else {
                        ...
                        if (pathName[mPendingEventItems[i].data.fd] != NULL) {
                            ProcessEvent(env, gl_class, pathName[mPendingEventItems[i].data.fd],
                                         curInotifyEvent->name, IN_ATTRIB);
                        } else {
                        ...
                        if (pathName[mPendingEventItems[i].data.fd] != NULL) {
                            ProcessEvent(env, gl_class, pathName[mPendingEventItems[i].data.fd],
                                         curInotifyEvent->name, IN_CLOSE_WRITE);
                        } else {
                        ...
                        if (pathName[mPendingEventItems[i].data.fd] != NULL) {
                            ProcessEvent(env, gl_class, pathName[mPendingEventItems[i].data.fd],
                                         curInotifyEvent->name, IN_CLOSE_NOWRITE);
                        } else {
                        ...
                        if (pathName[mPendingEventItems[i].data.fd] != NULL) {
                            ProcessEvent(env, gl_class, pathName[mPendingEventItems[i].data.fd],
                                         curInotifyEvent->name, IN_DELETE_SELF);
                        } else {
                        ...
                        if (pathName[mPendingEventItems[i].data.fd] != NULL) {
                            ProcessEvent(env, gl_class, pathName[mPendingEventItems[i].data.fd],
                                         curInotifyEvent->name, IN_MODIFY);
                        } else {
                        ...
                        if (pathName[mPendingEventItems[i].data.fd] != NULL) {
                            ProcessEvent(env, gl_class, pathName[mPendingEventItems[i].data.fd],
                                         curInotifyEvent->name, IN_MOVE_SELF);
                        } else {
                        ...
                        if (pathName[mPendingEventItems[i].data.fd] != NULL) {
                            ProcessEvent(env, gl_class, pathName[mPendingEventItems[i].data.fd],
                                         curInotifyEvent->name, IN_MOVED_FROM);
                        } else {
                        ...
                        if (pathName[mPendingEventItems[i].data.fd] != NULL) {
                            ProcessEvent(env, gl_class, pathName[mPendingEventItems[i].data.fd],
                                         curInotifyEvent->name, IN_MOVED_TO);
                        } else {
...
Java_com_jiangc_receiver_FileObserverJni_FileObserverInit(JNIEnv *env, jclass clazz, jstring path,
                                                          jint mask) {
 ...
 */
JNIEXPORT jint JNICALL
Java_com_jiangc_receiver_FileObserverJni_FileObserverDestroy(JNIEnv *env, jclass cls) {
...
Java_com_jiangc_receiver_FileObserverJni_error2String(JNIEnv *env, jclass clazz, jint err) {
    jstring str = (*env)->NewStringUTF(env, strerror(err));
    return str;
}
